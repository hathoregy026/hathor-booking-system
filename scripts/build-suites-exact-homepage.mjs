...
<style data-hathor-suites-palette>
  ...
  /* Gold accent glows (Springs used green radials). Keep modest so photos stay readable.
     Do NOT flatten .l-gallery__gradient layers — Springs’ soft depth comes from those
     stacked radials; gallery tokens are gold so the ambient shadow stays #B69F64-based. */
  ...
  /*
   * Suites hero gutters: solid 100% #B69F64 (no ink wash / vignette mud).
   * Keep slower mosaic drift; restore Springs ambient gradient layers via gold tokens.
   * Soft elevation shadows stay Springs’ original box-shadow filters, retinted to gold
   * in the CSS color pass (pure-black → rgba(182,159,100)) — no custom drop-shadow.
   */
  ...
  /* Springs’ last gradient layer was near-black vignette — retint to soft gold depth. */
  .l-gallery__gradient div:nth-child(5) {
    background: radial-gradient(
      circle,
      rgba(44, 40, 36, 0.45) 0,
      rgba(44, 40, 36, 0.2) 20%,
      rgba(44, 40, 36, 0) 70%
    ) !important;
  }
  /*
   * Hero mosaic — match Springs card field + clear contrast on gold gutters:
   * 1) Lock Springs 3:4 card box + object-fit cover (Hathor landscape webps were
   *    collapsing card height vs Springs 720×900 tiles — main size mismatch).
   * 2) Full opacity (stock .7 + gold wash muddied photos).
   * 3) Mild scale-up of the Springs -28° plane to eat empty gold corners without
   *    breaking lane spacing (over-expanding the plane fragmented the field).
   * 4) Soft ink edge vignette for cream title legibility — not a gold color cast.
   */
  .l-gallery {
  ...
  .l-gallery__item {
    /* Tighter gutters so gold lanes read like Springs’ thin dark seams. */
  ...
  /* Kill gold-on-photo wash from remapped Springs green radials; keep soft vignette. */
  ...
  @media (max-width: 480px) {
    ...
    .l-gallery__item .img-full img,
    .l-gallery__item img {
      filter: contrast(1.08) saturate(1.04) brightness(1.02) !important;
    }
    .l-gallery-container::after {
      background: radial-gradient(
        ellipse at 50% 45%,
        rgba(44, 40, 36, 0) 34%,
        rgba(44, 40, 36, 0.2) 80%,
        rgba(44, 40, 36, 0.32) 100%
      );
    }
  }
  /* Title/lede on gold hero — cream + shadow for clarity over mosaic. */
  .l-gallery__caption {
  ...
  .l-gallery__caption .h0 {
    color: #ece8df !important;
    text-shadow:
      0 1px 2px rgba(44, 40, 36, 0.75),
      0 4px 28px rgba(44, 40, 36, 0.55);
  }
  /*
