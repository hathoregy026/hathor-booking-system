...
<style data-hathor-suites-palette>
  ...
  /*
   * Hero mosaic — match Springs full-bleed field + clear contrast on gold gutters:
   * 1) Expand the -28° plane past empty corners (stock 100vw leaves gold voids).
   * 2) Lock Springs 3:4 card box + object-fit cover (Hathor landscape webps were
   *    collapsing card height vs Springs 720×900 tiles).
   * 3) Full opacity cards (stock .7 + gold wash made photos muddy).
  ...
  .l-gallery__item img {
    ...
    opacity: 1 !important;
    border-radius: 0 !important;
    /* Clarify photos without a gold cast */
    filter: contrast(1.08) saturate(1.06) !important;
  }
  /* Kill gold-on-photo wash from remapped Springs green radials; keep one soft vignette. */
  ...
  .l-gallery-container::after {
    ...
    background: radial-gradient(
      ellipse at 50% 45%,
      rgba(44, 40, 36, 0) 28%,
      rgba(44, 40, 36, 0.28) 78%,
      rgba(44, 40, 36, 0.42) 100%
    );
  }
  ...
  @media (max-width: 480px) {
    ...
    .l-gallery-container::after {
      background: radial-gradient(
        ellipse at 50% 45%,
        rgba(44, 40, 36, 0) 32%,
        rgba(44, 40, 36, 0.22) 80%,
        rgba(44, 40, 36, 0.34) 100%
      );
    }
  }
  ...
  .l-gallery__caption .h0 {
    color: #ece8df !important;
    text-shadow: 0 2px 24px rgba(44, 40, 36, 0.55);
  }
  /*
