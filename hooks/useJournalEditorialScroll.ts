...
type JournalEditorialScrollRefs = {
...
/**
 * Journal horizontal story: desktop scrubs a sticky track rightward;
 * tablet and phone stack the same scenes vertically.
 */
export function useJournalEditorialScroll({
  rootRef,
  runRef,
  trackRef,
}: JournalEditorialScrollRefs) {
  useEffect(() => {
    ...
    const progressBar = root.querySelector<HTMLElement>("[data-jn-progress]");
    const scenes = [...root.querySelectorAll<HTMLElement>(".jn-scene")];
    const flips = [...root.querySelectorAll<HTMLElement>("[data-jn-flip]")];
    ...
    let target = 0;
    let current = 0;
    html.setAttribute("data-journal-editorial", "");

    const applyFlips = (mode: "horizontal" | "vertical") => {
      flips.forEach((el) => {
        if (reduced.matches) {
          el.style.setProperty("--jn-flip", "1");
          return;
        }
    ...
    return () => {
      ...
      html.removeAttribute("data-journal-editorial");
      run.style.height = "";
