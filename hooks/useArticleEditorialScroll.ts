...
type ArticleEditorialScrollRefs = {
...
/**
 * Blog article horizontal story: desktop scrubs a sticky track rightward;
 * tablet and phone stack the same scenes vertically.
 */
export function useArticleEditorialScroll({
  rootRef,
  runRef,
  trackRef,
}: ArticleEditorialScrollRefs) {
  useEffect(() => {
    ...
    const progressBar = root.querySelector<HTMLElement>("[data-ar-progress]");
    const scenes = [...root.querySelectorAll<HTMLElement>(".ar-scene")];
    const flips = [...root.querySelectorAll<HTMLElement>("[data-ar-flip]")];
    ...
    html.setAttribute("data-article-editorial", "");

    const applyFlips = (mode: "horizontal" | "vertical") => {
      flips.forEach((el) => {
        if (reduced.matches) {
          el.style.setProperty("--ar-flip", "1");
          return;
        }
    ...
    return () => {
      ...
      html.removeAttribute("data-article-editorial");
      run.style.height = "";
