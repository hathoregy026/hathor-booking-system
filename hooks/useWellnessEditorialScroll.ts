...
type WellnessEditorialScrollRefs = {
...
/**
 * LuxuryHathor horizontal story for Wellness: desktop scrubs a sticky track;
 * at 950px and below (or reduced motion) the same scenes stack vertically.
 */
export function useWellnessEditorialScroll({
  rootRef,
  runRef,
  trackRef,
}: WellnessEditorialScrollRefs) {
  useEffect(() => {
    ...
    const progressBar = root.querySelector<HTMLElement>("[data-we-progress]");
    const scenes = [...root.querySelectorAll<HTMLElement>(".we-scene")];
    const flips = [...root.querySelectorAll<HTMLElement>("[data-we-flip]")];
    ...
    html.setAttribute("data-wellness-editorial", "");

    const applyFlips = (mode: "horizontal" | "vertical") => {
      flips.forEach((el) => {
        if (reduced.matches) {
          el.style.setProperty("--we-flip", "1");
          return;
        }
    ...
    return () => {
      ...
      html.removeAttribute("data-wellness-editorial");
      run.style.height = "";
