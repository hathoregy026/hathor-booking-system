...
type HighlightsEditorialScrollRefs = {
...
export function useHighlightsEditorialScroll({
  ...
  useEffect(() => {
    ...
    const progressBar = root.querySelector<HTMLElement>("[data-hl-progress]");
    const scenes = [...root.querySelectorAll<HTMLElement>(".hl-scene")];
    const flips = [...root.querySelectorAll<HTMLElement>("[data-hl-flip]")];
    ...
    const applyFlips = (mode: "horizontal" | "vertical") => {
      flips.forEach((el) => {
        if (reduced.matches) {
          el.style.setProperty("--hl-flip", "1");
          return;
        }
