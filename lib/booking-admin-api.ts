...
export async function applyStaffBookingAction(id: string, body: unknown, recordedBySession: string | null = null) {
  ...
  return administerBooking(id, action.type === "record-payment"
    ? { ...action, payment: { ...action.payment, recordedBySession: recordedBySession ?? undefined } }
    : action);
}
