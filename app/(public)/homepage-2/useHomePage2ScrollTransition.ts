/**
 * Homepage-2 scroll reveal — same engine as cruises with cream tokens + dome bleed.
 */
"use client";
...
export function useHomePage2ScrollTransition(refs: PageScrollTransitionRefs) {
  ...
  useLayoutEffect(() => {
    ...
    function applyProgress(p: number) {
      ...
      // Bleed sheet horizontally while the dome is still wide — removes side "ears".
      const scaleX = 1 + (1 - radiusProgress) * 0.12;

      trigger.classList.toggle("homepage-2--mask-phase", p >= MASK.start);
    ...
    return () => {
      ...
      ctx.revert();
      trigger.classList.remove("homepage-2--mask-phase");
