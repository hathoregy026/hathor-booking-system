-- A map selection may request one exact physical cabin. The existing type-only
-- journey remains unchanged; all selection, rate, overlap and snapshot checks
-- still run inside the same atomic hold transaction.
...
BEGIN
 ...
 FOR item IN SELECT value FROM jsonb_array_elements(requested_rooms) LOOP
  ...
  SELECT r.id INTO room_id FROM "Room" r JOIN "_CruiseRooms" cr ON cr."B"=r.id
   ...
   AND (item->>'roomId' IS NULL OR r.id=item->>'roomId')
   AND (item->>'roomId' IS NOT NULL OR NOT EXISTS(SELECT 1 FROM jsonb_array_elements(requested_rooms) wanted WHERE wanted->>'roomId'=r.id))
  ...
  IF room_id IS NULL THEN RAISE EXCEPTION USING ERRCODE='HB409',MESSAGE='The selected cabin or room combination is no longer available. No partial hold was created.'; END IF;
...
REVOKE ALL ON FUNCTION hathor_acquire_hold(text,jsonb,text,text) FROM PUBLIC, anon, authenticated;
